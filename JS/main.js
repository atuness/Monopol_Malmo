...
function getPos() {
    ...
    $.get("PHP/getCard.php", {})
        .done(function (data) {
            $("#btnPrison, #btn, #btnchans, #btnStation,.station, #btnTarning").css("display", "none")
            $("#pricelist").css("display", "flex")
            let card = JSON.parse(data)
            console.log(card[0].teamID)
            let theCardOwner = card[0].teamID
            ...
            //HÄR
                ...
                .done(function (data) {
                    ...
                    for (let i=0; i<whoDis.length; i++){
                        ...
                        $(".theOwner").html('Ägs av: ' + ownerName)
                    }
                })
            ...
            if (card[0].teamID == 0) {
                ...
                $("#streetID input[name='name']").val(code)
